...
interface LegacyAdapterFactoryEntryPoint {
    fun legacyScaleAdapterFactory(): LegacyScaleAdapter.Factory
}
 ...
 */
class LegacyScaleAdapter @AssistedInject constructor(
    @ApplicationContext private val applicationContext: Context,
    @Assisted private val bluetoothDriverInstance: BluetoothCommunication, // The specific driver instance
    ...
    @AssistedFactory
    interface Factory {
        fun create(bluetoothDriverInstance: BluetoothCommunication): LegacyScaleAdapter
    }
    ...
    private suspend fun provideUserDataToLegacyDriver() {
        try {
            ...
            var base: Int = settingsFacade.observeSetting(keyName, 0).first()
            if (base == 0) {
                base = Random.nextInt(100, 65535)
                settingsFacade.saveSetting(keyName, base)
