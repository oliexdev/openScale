...
public class BluetoothES26BBB extends BluetoothCommunication {
    private static final String TAG = "BluetoothES26BBB";
    ...
    private void parseNotifyPacket(byte[] data) {
        ...
        LogManager.d(TAG, String.format("Received measurement packet: %s", dataStr));

        if (!isChecksumValid(data)) {
            LogManager.d(TAG, String.format("Checksum of packet did not match. Ignoring measurement. Packet: %s", dataStr));
            return;
        }
        ...
        switch (action) {
            ...
            case 0x11:
                ...
                LogManager.d(TAG,
                        String.format("Received scale information. Power status: %d, Unit: %d, Precision: %d, Offline count: %d, Battery: %d",
            ...
            case 0x10:
                ...
                byte success = data[5];
                if (success == 1) {
                    LogManager.d(TAG, "Received success for operation");
                } else {
                    LogManager.d(TAG, "Received failure for operation");
                }
            default:
                LogManager.d(TAG, String.format( "Unknown action sent from scale: %x. Full packet: %s", action, dataStr));
                break;
        }
    }
    ...
    private boolean isChecksumValid(byte[] data) {
        if (data.length == 0) {
            LogManager.d(TAG, "Could not validate checksum because payload is empty");
            return false;
        }

        byte checksum = data[data.length - 1];
        byte sum = sumChecksum(data, 0, data.length - 1);
        LogManager.d(TAG, String.format("Comparing checksum (%x == %x)", sum, checksum));
        return sum == checksum;
    }
    ...
    private void handleMeasurementPayload(byte[] data) {
        LogManager.d(TAG, "Parsing measurement");
        ...
        if (measurementType != 0x01 && measurementType != 0x11) {
            ...
            LogManager.d(TAG, "Discarded measurement since it is not final");
            return;
        }

        LogManager.d(TAG, "Saving measurement");
        // Weight (in kg) is stored as big-endian in bytes 6 to 9
        long weightKg = ConverterUtils.fromUnsignedInt32Be(data, 6);
        // Resistance/Impedance is stored as big-endian in bytes 10 to 11
        int resistance = ConverterUtils.fromUnsignedInt16Be(data, 10);

        LogManager.d(TAG, String.format("Got measurement from scale. Weight: %d, Resistance: %d", weightKg, resistance));
    ...
    private void handleOfflineMeasurementPayload(byte[] data) {
        LogManager.d(TAG, "Parsing offline measurement");

        // Weight (in kg) is stored as big-endian in bytes 5 to 8
        long weightKg = ConverterUtils.fromUnsignedInt32Be(data, 5);
        // Resistance/Impedance is stored as big-endian in bytes 9 to 10
        int resistance = ConverterUtils.fromUnsignedInt16Be(data, 9);
        // Scale returns the seconds elapsed since the measurement as big-endian in bytes 11 to 14
        long secondsSinceMeasurement = ConverterUtils.fromUnsignedInt32Be(data, 11);
        long measurementTimestamp = System.currentTimeMillis() - secondsSinceMeasurement * 1000;

        LogManager.d(TAG, String.format("Got offline measurement from scale. Weight: %d, Resistance: %d, Timestamp: %tc", weightKg, resistance, measurementTimestamp));
    ...
    private void acknowledgeOfflineMeasurement() {
        ...
        LogManager.d(TAG, "Acknowledge offline measurement");
    }
