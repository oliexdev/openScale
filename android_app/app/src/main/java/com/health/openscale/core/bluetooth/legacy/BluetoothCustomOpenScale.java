...
public class BluetoothCustomOpenScale extends BluetoothCommunication {
    private final String TAG = "BluetoothCustomOpenScale";
    ...
    private void parseBtString(String btString) {
        ...
        switch (btString.charAt(1)) {
            case 'I':
                LogManager.d(TAG, String.format("MCU Information: %s", btMsg));
                break;
            case 'E':
                LogManager.e(TAG, String.format("MCU Error: %s", btMsg), null);
                break;
            case 'S':
                LogManager.d(TAG, String.format("MCU stored data size: %s", btMsg));
                break;
            case 'F':
                LogManager.d(TAG, "All data sent");
                clearEEPROM();
                disconnect();
                break;
            case 'D':
