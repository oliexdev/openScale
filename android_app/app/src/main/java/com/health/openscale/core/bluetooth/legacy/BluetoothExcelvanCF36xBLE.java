...
public class BluetoothExcelvanCF36xBLE extends BluetoothCommunication {
    ...
    private void parseBytes(byte[] weightBytes) {
        float weight = ConverterUtils.fromUnsignedInt16Be(weightBytes, 4) / 10.0f;
        float fat = ConverterUtils.fromUnsignedInt16Be(weightBytes, 6) / 10.0f;
        float bone = (weightBytes[8] & 0xFF) / 10.0f;
        float muscle = ConverterUtils.fromUnsignedInt16Be(weightBytes, 9) / 10.0f;
        float visceralFat = weightBytes[11] & 0xFF;
        float water = ConverterUtils.fromUnsignedInt16Be(weightBytes, 12) / 10.0f;
        float bmr = ConverterUtils.fromUnsignedInt16Be(weightBytes, 14);
