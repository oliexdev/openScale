...
public class BluetoothBeurerSanitas extends BluetoothCommunication {
    private static final String TAG = "BluetoothBeurerSanitas";
    ...
    private long decodeUserId(byte[] data, int offset) {
        long high = ConverterUtils.fromUnsignedInt32Be(data, offset);
        long low = ConverterUtils.fromUnsignedInt32Be(data, offset + 4);
        return (high << 32) | low;
    }
    ...
    private byte[] encodeUserId(RemoteUser remoteUser) {
        ...
        ConverterUtils.toInt32Be(data, 0, uid >> 32);
        ConverterUtils.toInt32Be(data, 4, uid & 0xFFFFFFFF);
        return data;
    }
    ...
    protected boolean onNextStep(int stepNr) {
        switch (stepNr) {
            ...
            case 1:
                ...
                LogManager.d(TAG, "Sending command: ID_START_NIBBLE_INIT");
            ...
            case 2:
                ...
                LogManager.d(TAG, "Sending command: ID_START_NIBBLE_SET_TIME");
                sendAlternativeStartCode(ID_START_NIBBLE_SET_TIME, ConverterUtils.toInt32Be(unixTime));
                break;
            case 3:
                // We will be waiting for data in state 3
                waitForDataInStep = 3;
                // Request scale status and wait for ack
                LogManager.d(TAG, "Sending command: CMD_SCALE_STATUS");
            ...
            case 4:
                ...
                LogManager.d(TAG, "Sending command: CMD_USER_LIST");
            ...
            case 5:
                ...
                if (currentRemoteUser != null) {
                    ...
                    waitForDataInStep = 5;
                    LogManager.d(TAG, String.format("Request saved measurements (CMD_GET_SAVED_MEASUREMENTS) for %s", currentRemoteUser.name));
            ...
            case 7:
                waitForDataInStep = 7;
                LogManager.d(TAG, "Sending command: CMD_USER_DETAILS");
            ...
            case 8:
                ...
                if( storedMeasurement.measurementData != null ) {
                    LogManager.d(TAG, "Reached state 8 (end) and still have saved data available. Storing now.");
                    if( currentRemoteUser != null ) {
                        LogManager.i(TAG, "User has been identified in the meantime, so store the data for them.");
                    ...
                    LogManager.d(TAG, "All finished, nothing to do.");
                    return false;
                }
                break;
            default:
                // Finish init if everything is done
                LogManager.d(TAG, "End of state flow reached.");
                return false;
        }
    ...
    public void onBluetoothNotify(UUID characteristic, byte[] value) {
        ...
        if (data == null || data.length == 0) {
            LogManager.d(TAG, "Received empty message.");
            return;
        }
        ...
        if (data[0] == getAlternativeStartByte(ID_START_NIBBLE_INIT)) {
            ...
            if( waitForDataInStep == 1 ) {
                LogManager.d(TAG, "Received init ack (ID_START_NIBBLE_INIT) from scale; scale is ready");
            }
            else {
                LogManager.d(TAG, "Received init ack (ID_START_NIBBLE_INIT) from scale in wrong state. Scale or app is confused. Continue in state 2.");
                jumpNextToStepNr( 2 );
            }
        ...
        if (data[0] != startByte) {
            LogManager.d(TAG, String.format("Got unknown start byte 0x%02x", data[0]));
            return;
        }
        ...
        try {
            switch (data[1]) {
                case CMD_USER_INFO:
                    LogManager.d(TAG, "Received: CMD_USER_INFO");
                    processUserInfo(data);
                    break;
                case CMD_SAVED_MEASUREMENT:
                    LogManager.d(TAG, "Received: CMD_SAVED_MEASUREMENT");
                    processSavedMeasurement(data);
                    break;
                case CMD_WEIGHT_MEASUREMENT:
                    LogManager.d(TAG, "Received: CMD_WEIGHT_MEASUREMENT");
                    processWeightMeasurement(data);
                    break;
                case CMD_MEASUREMENT:
                    LogManager.d(TAG, "Received: CMD_MEASUREMENT");
                    processMeasurement(data);
                    break;
                case CMD_SCALE_ACK:
                    LogManager.d(TAG, "Received: CMD_SCALE_ACK");
                    processScaleAck(data);
                    break;
                default:
                    LogManager.d(TAG, String.format("Unknown command 0x%02x", data[1]));
                    break;
            }
        }
    ...
    private void processUserInfo(byte[] data) {
        ...
        if (remoteUsers.size() == current - 1) {
            ...
            LogManager.d(TAG, String.format("Received user %d/%d: %s (%d)", current, count, name, year));
        }

        LogManager.d(TAG, "Sending ack for CMD_USER_INFO");
        sendAck(data);

        if (current != count) {
            LogManager.d(TAG, "Not all users received, waiting for more...");
        ...
        if( waitForDataInStep != 4 ) {
            LogManager.d(TAG, "Received final CMD_USER_INFO in wrong state...");
            if( waitForDataInStep >= 0 ){
                LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                // We are in the wrong state.
                // This may happen, so let's just retry whatever we did before.
                jumpBackOneStep();
            }
            else {
                LogManager.d(TAG, "...ignored, no data expected.");
            }
        }
        // All data received, no more waiting.
        waitForDataInStep = -1;
    ...
    private void processMeasurementData(byte[] data, int offset, boolean firstPart, boolean processingSavedMeasurements) {
        if (firstPart) {
            if( measurementData != null ) LogManager.d(TAG, "Discarding existing data.");
            measurementData = Arrays.copyOfRange(data, offset, data.length);
            return;
        }

        if( measurementData == null ) {
            LogManager.d(TAG, "Received second measurement part without receiving first part before. Discarding data.");
            return;
        }
        ...
        if( currentRemoteUser != null && (readyForData || processingSavedMeasurements) ) {
            LogManager.d(TAG, "Measurement complete, user identified and app ready: Storing data.");
            addMeasurement(measurementData, currentRemoteUser.localUserId);
            ...
            if( storedMeasurement.measurementData != null ) {
                ...
                if( currentRemoteUser.remoteUserId == storedMeasurement.storedUid ) {
                    ...
                    if( ConverterUtils.fromUnsignedInt32Be(measurementData, 0) == ConverterUtils.fromUnsignedInt32Be(storedMeasurement.measurementData, 0) ) {
                        // Then delete the unsaved data because it is already part of the received saved data
                        LogManager.d(TAG, "Discarding data saved for later, because it is already part of the received saved data from the scale.");
                        storedMeasurement.measurementData = null;
                    }
                }
            }
            ...
            if( !readyForData ) {
                LogManager.d(TAG, "New measurement complete, but not stored, because app not ready: Saving data for later.");
            }
            else {
                LogManager.d(TAG, "New measurement complete, but not stored, because user not identified: Saving data for later.");
            }
            ...
            LogManager.d(TAG, "Received saved measurement, but do not know for what user. This should not happen. Discarding data.");
            measurementData = null;
        }
    }
    ...
    private void processSavedMeasurement(byte[] data) {
        ...
        LogManager.d(TAG, String.format("Received part %d (of 2) of saved measurement %d of %d.", current % 2 == 1 ? 1 : 2, current / 2, count / 2));

        processMeasurementData(data, 4, current % 2 == 1, true);

        LogManager.d(TAG, "Sending ack for CMD_SAVED_MEASUREMENT");
        sendAck(data);

        if (current != count) {
            LogManager.d(TAG, "Not all parts / saved measurements received, waiting for more...");
            // More data should be incoming, so make sure we wait
            stopMachineState();
            return;
        }

        LogManager.i(TAG, "All saved measurements received.");

        // This message should only be received in step 5
        if( waitForDataInStep != 5 ) {
            LogManager.d(TAG, "Received final CMD_SAVED_MEASUREMENT in wrong state...");
            if( waitForDataInStep >= 0 ){
                LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                // We are in the wrong state.
                // This may happen, so let's just retry whatever we did before.
        ...
        LogManager.d(TAG, String.format("Deleting saved measurements (CMD_DELETE_SAVED_MEASUREMENTS) for %s", currentRemoteUser.name));
    ...
    private void processWeightMeasurement(byte[] data) {
        ...
        LogManager.i(TAG, String.format("Active measurement, stable weight: %.2f", weight));
    }

    private void processMeasurement(byte[] data) {
        int count = data[2] & 0xFF;
        int current = data[3] & 0xFF;
        LogManager.d(TAG, String.format("Received measurement part %d of %d.", current, count ));

        if (current == 1) {
            long uid = decodeUserId(data, 5);
            LogManager.d(TAG, String.format("Receiving measurement data for remote UID %d.", uid));
            ...
            for (RemoteUser remoteUser : remoteUsers) {
                if (remoteUser.remoteUserId == uid) {
                    currentRemoteUser = remoteUser;
                    LogManager.d(TAG, String.format("Local user %s matches remote UID %d.", currentRemoteUser.name, uid));
                    break;
                }
            }
            if( currentRemoteUser == null ) {
                LogManager.d(TAG, String.format("No local user identified for remote UID %d.", uid));
            }
        }
        else {
        ...
        LogManager.d(TAG, "Sending ack for CMD_MEASUREMENT");
        sendAck(data);

        if (current != count) {
            LogManager.d(TAG, "Not all measurement parts received, waiting for more...");
            // More data should be incoming, so make sure we wait
            stopMachineState();
            return;
        }

        LogManager.i(TAG, "All measurement parts received.");

        // Delete saved measurement, but only when we processed it before
        if (currentRemoteUser != null && readyForData ) {
            LogManager.d(TAG, "Sending command: CMD_DELETE_SAVED_MEASUREMENTS");
            sendCommand(CMD_DELETE_SAVED_MEASUREMENTS, encodeUserId(currentRemoteUser));
            // We sent a new command, so make sure we wait
            stopMachineState();
        }
        // This message should only be received in step 6 and 8
            ...
            LogManager.d(TAG, "Received final CMD_MEASUREMENT in wrong state...");
            if( waitForDataInStep >= 0 ){
                LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                // We are in the wrong state.
                // This may happen, so let's just retry whatever we did before.
    ...
    private void processScaleAck(byte[] data) {
        switch (data[2]) {
            case CMD_SCALE_STATUS:
                LogManager.d(TAG, "ACK type: CMD_SCALE_STATUS");
                ...
                LogManager.d(TAG, String.format("Battery level: %d; threshold: weight=%.2f, body fat=%.2f;"
                ...
                if (requestedUnit != currentUnit) {
                    ...
                    if( waitForDataInStep != 3 ) {
                        LogManager.d(TAG, "Received ACK for CMD_SCALE_STATUS in wrong state...");
                        if( waitForDataInStep >= 0 ){
                            LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                            // We are in the wrong state.
                            // This may happen, so let's just retry whatever we did before.
            ...
            case CMD_SET_UNIT:
                LogManager.d(TAG, "ACK type: CMD_SET_UNIT");
                if (data[3] == 0) {
                    LogManager.d(TAG, "Scale unit successfully set");
                }
                // This should only be received in step 3
                if( waitForDataInStep != 3 ) {
                    LogManager.d(TAG, "Received ACK for CMD_SET_UNIT in wrong state...");
                    if( waitForDataInStep >= 0 ){
                        LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                        // We are in the wrong state.
                        // This may happen, so let's just retry whatever we did before.
            ...
            case CMD_USER_LIST:
                LogManager.d(TAG, "ACK type: CMD_USER_LIST");
                int userCount = data[4] & 0xFF;
                int maxUserCount = data[5] & 0xFF;
                LogManager.d(TAG, String.format("Have %d users (max is %d)", userCount, maxUserCount));
                if (userCount == 0) {
                    // We expect no more data, because there are no stored users.
                    // This message should only be received in state 4.
                    if( waitForDataInStep != 4 ) {
                        LogManager.d(TAG, "Received ACK for CMD_USER_LIST in wrong state...");
                        if( waitForDataInStep >= 0 ){
                            LogManager.d(TAG, "...while waiting for other data.");
                            // We are in the wrong state.
                            // This may happen, so let's just retry whatever we did before.
                            jumpBackOneStep();
                        }
            ...
            case CMD_GET_SAVED_MEASUREMENTS:
                LogManager.d(TAG, "ACK type: CMD_GET_SAVED_MEASUREMENTS");
                int measurementCount = data[3] & 0xFF;
                LogManager.d(TAG, String.format("Received ACK for CMD_GET_SAVED_MEASUREMENTS for %d measurements.", measurementCount/2));
                if (measurementCount == 0) {
                    // We expect no more data, because there are no measurements.
                    readyForData = true;
                    // This message should only be received in step 5.
                    if( waitForDataInStep != 5 ) {
                        LogManager.d(TAG, "Received ACK for CMD_GET_SAVED_MEASUREMENTS in wrong state...");
                        if( waitForDataInStep >= 0 ){
                            LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                            // We are in the wrong state.
                            // This may happen, so let's just retry whatever we did before.
            ...
            case CMD_DELETE_SAVED_MEASUREMENTS:
                LogManager.d(TAG, "ACK type: CMD_DELETE_SAVED_MEASUREMENTS");
                if (data[3] == 0) {
                    LogManager.d(TAG, "Saved measurements successfully deleted for user " + currentRemoteUser.name);
                }
                // This message should only be received in state 5, 6 or 8
                if( waitForDataInStep != 5 && waitForDataInStep != 6 && waitForDataInStep != 8 ) {
                    LogManager.d(TAG, "Received ACK for CMD_DELETE_SAVED_MEASUREMENTS in wrong state...");
                    if( waitForDataInStep >= 0 ){
                        LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                        // We are in the wrong state.
                        // This may happen, so let's just retry whatever we did before.
            ...
            case CMD_USER_ADD:
                LogManager.d(TAG, "ACK type: CMD_USER_ADD");
                // This message should only be received in state 6
                if( waitForDataInStep != 6 ) {
                    LogManager.d(TAG, "Received ACK for CMD_USER_ADD in wrong state...");
                    if( waitForDataInStep >= 0 ){
                        LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                        // We are in the wrong state.
                        // This may happen, so let's just retry whatever we did before.
                ...
                if (data[3] == 0) {
                    ...
                    // If we have unprocessed data available, store it now.
                    if( storedMeasurement.measurementData != null ) {
                        LogManager.d(TAG, "User identified, storing unprocessed data.");
            ...
            case CMD_DO_MEASUREMENT:
                LogManager.d(TAG, "ACK type: CMD_DO_MEASUREMENT");
                if (data[3] != 0) {
                    LogManager.d(TAG, "Measure command rejected.");
                    ...
                    if( waitForDataInStep != 6 && waitForDataInStep != 8 ) {
                        LogManager.d(TAG, "Received ACK for CMD_DO_MEASUREMENT in wrong state...");
                        if( waitForDataInStep >= 0 ){
                            LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                            // We are in the wrong state.
                            // This may happen, so let's just retry whatever we did before.
            ...
            case CMD_USER_DETAILS:
                LogManager.d(TAG, "ACK type: CMD_USER_DETAILS");
                if (data[3] == 0) {
                    ...
                    LogManager.d(TAG, String.format("Name: %s, Birthday: %d-%02d-%02d, Height: %d, Sex: %s, activity: %d",
                ...
                if( waitForDataInStep != 7 ) {
                    LogManager.d(TAG, "Received ACK for CMD_USER_DETAILS in wrong state...");
                    if( waitForDataInStep >= 0 ){
                        LogManager.d(TAG, "...while waiting for other data. Retrying last step.");
                        // We are in the wrong state.
                        // This may happen, so let's just retry whatever we did before.
            ...
            default:
                LogManager.d(TAG, String.format("Unhandled scale ack for command 0x%02x", data[2]));
                break;
        }
    }
    ...
    private void addMeasurement(byte[] data, int userId) {
        long timestamp = ConverterUtils.fromUnsignedInt32Be(data, 0) * 1000;
        float weight = getKiloGram(data, 4);
        int impedance = ConverterUtils.fromUnsignedInt16Be(data, 6);
        float fat = getPercent(data, 8);
        float water = getPercent(data, 10);
        float muscle = getPercent(data, 12);
        float bone = getKiloGram(data, 14);
        int bmr = ConverterUtils.fromUnsignedInt16Be(data, 16);
        int amr = ConverterUtils.fromUnsignedInt16Be(data, 18);
        float bmi = ConverterUtils.fromUnsignedInt16Be(data, 20) / 10.0f;
    ...
    private void createRemoteUser(ScaleUser scaleUser) {
        ...
        LogManager.d(TAG, "Sending command: CMD_USER_ADD");
