...
public class BluetoothOneByone extends BluetoothCommunication {
    private final String TAG = "BluetoothOneByone";
    ...
    public void onBluetoothNotify(UUID characteristic, byte[] value) {
        ...
        if (data.length >= 11 && data[0] == (byte)0xcf) {
            ...
            LogManager.d(TAG, String.format("received bytes [%s]", byteInHex(data)));
    ...
    private void parseBytes(byte[] weightBytes) {
        float weight = ConverterUtils.fromUnsignedInt16Le(weightBytes, 3) / 100.0f;
        ...
        if (dateTimePresent) {
            ...
            dateTime.set(ConverterUtils.fromUnsignedInt16Be(weightBytes, 11),
        ...
        LogManager.d(TAG, String.format("received bytes [%s]", byteInHex(weightBytes)));
        LogManager.d(TAG, String.format("received decoded bytes [weight: %.2f, impedanceValue: %f]", weight, impedanceValue));
        LogManager.d(TAG, String.format("user [%s]", scaleUser));
        ...
        try {
            ...
            LogManager.d(TAG, String.format("scale measurement [%s]", scaleBtData));
            ...
            if (historicMeasurement) {
                LogManager.d(TAG, String.format("invalid time-stamp: year %d, month %d, day %d, hour %d, minute %d, second %d",
                        ConverterUtils.fromUnsignedInt16Be(weightBytes, 11),
