...
public class BluetoothOKOK2 extends BluetoothCommunication {
    private static final String TAG = "BluetoothOKOK2";
    ...
    static String convertNoNameToDeviceName(SparseArray<byte[]> manufacturerSpecificData) { // TODO implement in kotlin
        int vendorIndex = -1;
    ...
    private final BluetoothCentralManagerCallback btCallback = new BluetoothCentralManagerCallback() {
        ...
        public void onDiscoveredPeripheral(@NotNull BluetoothPeripheral peripheral, @NotNull ScanResult scanResult) {
            ...
            LogManager.d(TAG, String.format("manufacturerSpecificData: [VID=%04x] %s", vendorIndex, sb.toString()));
            ...
            switch ((data[IDX_ATTRIB] >> 3) & 3) {
                ...
                case UNIT_LB: {
                    float val = ((data[IDX_WEIGHT_MSB] & 0xff) << 8) | (data[IDX_WEIGHT_LSB] & 0xff);
                    weight = ConverterUtils.toKilogram(val / divider, WeightUnit.LB);
                    break;
                }
                case UNIT_STLB: {
                    float val = data[IDX_WEIGHT_MSB] /*ST*/ + data[IDX_WEIGHT_LSB] /*LB*/ / divider / 14f;
                    weight = ConverterUtils.toKilogram(val, WeightUnit.ST);
                    break;
                }
            }
