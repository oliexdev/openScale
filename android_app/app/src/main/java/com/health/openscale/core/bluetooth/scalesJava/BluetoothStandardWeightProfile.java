...
public abstract class BluetoothStandardWeightProfile extends BluetoothCommunication {
    private final String TAG = "BluetoothStandardWeightProfile";
    ...
    public BluetoothStandardWeightProfile(Context context) {
        ...
        this.selectedUser = getSelectedScaleUser();
    ...
    protected boolean onNextStep(int stepNr) {
        ...
        if (stepNr > SM_STEPS.MAX_STEP.ordinal()) {
            LogManager.d(TAG,  "WARNING: stepNr == " + stepNr + " outside range, must be from 0 to " + SM_STEPS.MAX_STEP.ordinal());
            stepNr = SM_STEPS.MAX_STEP.ordinal();
        }
        SM_STEPS step = SM_STEPS.values()[stepNr];
        LogManager.d(TAG, "stepNr: " + stepNr + " " + step);
    ...
    public void onBluetoothNotify(UUID characteristic, byte[] value) {
        ...
        if(characteristic.equals(BluetoothGattUuid.CHARACTERISTIC_CURRENT_TIME)) {
            ...
            LogManager.d(TAG, String.format("Received device time: %s", currentTime));
        }
            ...
            LogManager.d(TAG, String.format("Received manufacturer: %s", manufacturer));
        }
        else if(characteristic.equals(BluetoothGattUuid.CHARACTERISTIC_MODEL_NUMBER_STRING)) {
            String modelNumber = parser.getStringValue(0);
            LogManager.d(TAG, String.format("Received modelnumber: %s", modelNumber));
        }
            ...
            LogManager.d(TAG, String.format("Notification from CHARACTERISTIC_CHANGE_INCREMENT, value: %s", increment));
            resumeMachineState();
        }
        else {
            LogManager.d(TAG, String.format("Notification from unhandled characteristic: %s, value: [%s]",
        ...
        if(value[0]==UDS_CP_RESPONSE) {
            switch (value[1]) {
                case UDS_CP_LIST_ALL_USERS:
                    LogManager.d(TAG, "UDS_CP_LIST_ALL_USERS value [" + byteInHex(value) + "]");
                    break;
                case UDS_CP_REGISTER_NEW_USER:
                    if (value[2] == UDS_CP_RESP_VALUE_SUCCESS) {
                        ...
                        LogManager.e(TAG, "UDS_CP_REGISTER_NEW_USER: ERROR: could not register new scale user, code: " + value[2], null);
                    }
                    break;
                case UDS_CP_CONSENT:
                    if (registerNewUser) {
                        LogManager.d(TAG, "UDS_CP_CONSENT: registerNewUser==true, value[2] == " + value[2]);
                        resumeMachineState();
                        break;
                    }
                    if (value[2] == UDS_CP_RESP_VALUE_SUCCESS) {
                    ...
                    break;
                default:
                    LogManager.e(TAG, "CHARACTERISTIC_USER_CONTROL_POINT: Unhandled response code "
                            + value[1] + " value [" + byteInHex(value) + "]", null);
                    break;
            }
        }
        else {
            LogManager.d(TAG, "CHARACTERISTIC_USER_CONTROL_POINT: non-response code " + value[0]
    ...
    protected ScaleMeasurement weightMeasurementToScaleMeasurement(byte[] value) {
        ...
        LogManager.d(TAG, String.format(prefix + "value: [%s]", byteInHex(value)));
        BluetoothBytesParser parser = new BluetoothBytesParser(value);
        ...
        LogManager.d(TAG, String.format(prefix + "flags: 0x%02x ", flags)
                + "[" + (isKg ? "SI" : "Imperial")
        ...
        LogManager.d(TAG, prefix+ "weight: " + weightValue);
        scaleMeasurement.setWeight(weightValue);

        if(timestampPresent) {
            Date timestamp = parser.getDateTime();
            LogManager.d(TAG, prefix + "timestamp: " + timestamp.toString());
        ...
        if(userIDPresent) {
            ...
            LogManager.d(TAG, String.format(prefix + "scale user index: %d (app user id: %d)", scaleUserIndex, userID));
            if (userID != -1) {
        ...
        if(bmiAndHeightPresent) {
            ...
            LogManager.d(TAG, prefix + "BMI: " + BMI);
            float heightInMeters = parser.getIntValue(BluetoothBytesParser.FORMAT_UINT16) * 0.001f;
            LogManager.d(TAG, prefix + "heightInMeters: " + heightInMeters);
        }

        LogManager.d(TAG, String.format("Got weight: %s", weightValue));
    ...
    protected ScaleMeasurement bodyCompositionMeasurementToScaleMeasurement(byte[] value) {
        ...
        LogManager.d(TAG, prefix + "bodyFatPercentage: " + bodyFatPercentage);
        ...
        if (bmrPresent) {
            ...
            LogManager.d(TAG, prefix + "bmrInJoules: " + bmrInJoules + " bmrInKcal: " + bmrInKcal);
        }
        ...
        if (musclePercentagePresent) {
            ...
            LogManager.d(TAG, prefix + "musclePercentage: " + musclePercentage);
        ...
        if (muscleMassPresent) {
            ...
            LogManager.d(TAG, prefix + "muscleMass: " + muscleMass);
        }
        ...
        if (fatFreeMassPresent) {
            ...
            LogManager.d(TAG, prefix + "fatFreeMass: " + fatFreeMass);
        }
        ...
        if (softLeanMassPresent) {
            ...
            LogManager.d(TAG, prefix + "softLeanMass: " + softLeanMass);
        }
        ...
        if (bodyWaterMassPresent) {
            ...
            LogManager.d(TAG, prefix + "bodyWaterMass: " + bodyWaterMass);
        ...
        if (impedancePresent) {
            ...
            LogManager.d(TAG, prefix + "impedance: " + impedance);
        }
        ...
        if (weightPresent) {
            ...
            LogManager.d(TAG, prefix + "weightValue: " + weightValue);
            scaleMeasurement.setWeight(weightValue);
        }
        else {
        ...
        if (heightPresent) {
            ...
            LogManager.d(TAG, prefix + "heightValue: " + heightValue);
        }

        if (multiPacketMeasurement) {
            LogManager.e(TAG, prefix + "multiPacketMeasurement not supported!", null);
        }

        LogManager.d(TAG, String.format("Got body composition: %s", byteInHex(value)));
        return scaleMeasurement;
    }
    ...
    protected void registerUser(int consentCode) {
        ...
        LogManager.d(TAG, String.format("registerUser consentCode: %d", consentCode));
    ...
    protected void setUser(int userIndex, int consentCode) {
        ...
        LogManager.d(TAG, String.format("setUser userIndex: %d, consentCode: %d", userIndex, consentCode));
        writeBytes(BluetoothGattUuid.SERVICE_USER_DATA, BluetoothGattUuid.CHARACTERISTIC_USER_CONTROL_POINT, parser.getValue());
    }
    ...
    protected void writeActivityLevel() {
        LogManager.d(TAG, "Write user activity level not implemented!");
    }

    protected void writeInitials() {
        LogManager.d(TAG, "Write user initials not implemented!");
    }

    protected void setChangeIncrement() {
        BluetoothBytesParser parser = new BluetoothBytesParser();
        int i = 1;
        LogManager.d(TAG, String.format("Setting Change increment to %s", i));
    ...
    protected void requestMeasurement() {
        LogManager.d(TAG, "Take measurement command not implemented!");
    }
    ...
    protected void handleVendorSpecificUserList(byte[] value) {
            LogManager.d(TAG, String.format("Got user data: <%s>", byteInHex(value)));
            BluetoothBytesParser parser = new BluetoothBytesParser(value);
            ...
            if (userListStatus == 2) {
                ...
                for (int i = 0; i < scaleUserList.size(); i++) {
                    if (i == 0) {
                        LogManager.d(TAG, "scale user list:");
                    }
