...
public class BluetoothTrisaBodyAnalyze extends BluetoothCommunication {
    private static final String TAG = "BluetoothTrisaBodyAnalyze";
    ...
    public void connect(String hwAddress) {
        LogManager.i(TAG, String.format("connect(\"%s\")", hwAddress));
    ...
    protected void onBluetoothNotify(UUID characteristic, byte[] value) {
        ...
        LogManager.i(TAG, String.format("onBluetoothdataChange() characteristic=%s value=%s", characteristic, byteInHex(value)));
        if (UPLOAD_COMMAND_CHARACTERISTIC_UUID.equals(characteristic)) {
            if (value.length == 0) {
                LogManager.w(TAG, "Missing command byte!", null);
                return;
            }
            ...
            switch (command) {
                ...
                case UPLOAD_CHALLENGE:
                    ...
                    break;
                default:
                    LogManager.w(TAG, String.format("Unknown command byte received: %d", command), null);
            }
            return;
        }
        ...
        LogManager.w(TAG, String.format("Unknown characteristic changed: %s", characteristic), null);
    }

    private void onPasswordReceived(byte[] data) {
        if (data.length < 5) {
            LogManager.w(TAG, String.format("Password data too short"), null);
            return;
        }
        password = ConverterUtils.fromSignedInt32Le(data, 1);
        if (deviceId == null) {
            LogManager.w(TAG, String.format("Can't save password: device id not set!"),null);
        } else {
            LogManager.i(TAG, String.format("Saving password '%08x' for device id '%s'", password, deviceId));
    ...
    private void onChallengeReceived(byte[] data) {
        if (data.length < 5) {
            LogManager.w(TAG, "Challenge data too short", null);
            return;
        }
        if (password == null) {
        ...
        int challenge = ConverterUtils.fromSignedInt32Le(data, 1);
    ...
    private void onScaleMeasurumentReceived(byte[] data) {
        ...
        if (measurement == null) {
            LogManager.w(TAG, String.format("Failed to parse scale measure measurement data: %s", byteInHex(data)), null);
            return;
        }
    ...
    public ScaleMeasurement parseScaleMeasurementData(byte[] data, ScaleUser user) {
        ...
        int deviceTimestamp = ConverterUtils.fromSignedInt32Le(data, 5);
    ...
    private void writeCommand(byte commandByte, int argument) {
        ...
        bytes[0] = commandByte;
        ConverterUtils.toInt32Le(bytes, 1, argument);
        writeCommandBytes(bytes);
    }

    private void writeCommandBytes(byte[] bytes) {
        LogManager.d("writeCommand bytes=%s", byteInHex(bytes));
    ...
    private static Integer loadDevicePassword(Context context, String deviceId) {
        ...
        try {
            ...
            LogManager.w(TAG,"Password preference value is not an integer.", e);
            return null;
        }
    }
    ...
    public float getBase10Float(byte[] data, int offset) {
        int mantissa = ConverterUtils.fromUnsignedInt24Le(data, offset);
