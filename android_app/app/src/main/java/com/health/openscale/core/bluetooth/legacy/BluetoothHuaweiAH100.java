...
public class BluetoothHuaweiAH100 extends BluetoothCommunication {
    private static final String TAG = "BluetoothHuaweiAH100";
...
///////////////////////////////////////////////////////////////////
    ...
    protected boolean onNextStep(int stepNr) {
        ...
        switch (step) {
            case INIT:
                // wait scale wake up
                LogManager.d(TAG, "AH100::onNextStep step 0 = set notification");
            ...
            case AUTHORISE:
                ...
                // authorize in scale
                LogManager.d(TAG, "AH100::onNextStep  = authorize on scale");
                triesToAuth++;
                AHcmdAutorise();
                stopMachineState();
                break;
            case  SCALE_UNIT:
                LogManager.d(TAG, "AH100::onNextStep  = set scale unit");
                AHcmdSetUnit();
                stopMachineState();
                break;
            case  SCALE_TIME:
                LogManager.d(TAG, "AH100::onNextStep  = set scale time");
                AHcmdDate();
                stopMachineState();
                break;
            case USER_INFO:
                LogManager.d(TAG, "AH100::onNextStep  = send user info to scale");
                if ( !authorised ) {
                ...
                break;
            case SCALE_VERSION:
                LogManager.d(TAG, "AH100::onNextStep  = request scale version");
                if ( !authorised ) {
                    jumpNextToStepNr( STEPS.AUTHORISE.ordinal() );
                    break;
                }
            ...
            case WAIT_MEASUREMENT:
                ...
                break;
            case READ_HIST:
                LogManager.d(TAG, "AH100::onNextStep  = read history record from scale");
                if ( !authorised ) {
                    jumpNextToStepNr( STEPS.AUTHORISE.ordinal() );
                    break;
                }
                ...
                break;
            case READ_HIST_NEXT:
                LogManager.d(TAG, "AH100::onNextStep  = read NEXT history record from scale");
                if ( !authorised ) {
                    jumpNextToStepNr( STEPS.AUTHORISE.ordinal() );
                    break;
                }
                ...
                break;
            case EXIT:
                LogManager.d(TAG, "AH100::onNextStep  = Exit");
                authorised = false;
                scaleWakedUp = false;
                stopHeartBeat();
                disconnect();
            ...
            case EXIT2:
                ...
                stopHeartBeat();
                disconnect();
                LogManager.d(TAG, "AH100::onNextStep  = BIND Exit");
            default:
                return false;
        }
...
///////////////////////////////////////////////////////////////////
    ...
    public void onBluetoothNotify(UUID characteristic, byte[] value) {
        ...
        byte cmdlength = 0;
        LogManager.d(TAG, String.format("AH100::onBluetoothNotify uuid: %s", characteristic.toString()));

        if (data != null && data.length > 2) {
            LogManager.d(TAG, String.format("===> New NOTIFY hex data: %s", byteInHex(data)));
                ...
                switch (data[2]) {
                    case AH100_NOTIFICATION_WAKEUP:
                        ...
                        if (getStepNr() - 1 == STEPS.INIT_W.ordinal() ) {
                            LogManager.d(TAG, "AH100::onNotify = Scale is waked up in Init-stage");
                            startHeartBeat();
                            resumeMachineState();
                            break;
                        }
...
//                            break;
//                        }
                        LogManager.d(TAG, "AH100::onNotify = Scale is waked up");
//                        authorised = false;
//                        jumpNextToStepNr(STEPS.AUTHORISE.ordinal());
//                        resumeMachineState();
                        break;
                    ...
                    case AH100_NOTIFICATION_SCALE_VERSION:
                        ...
                        LogManager.d(TAG, String.format("Get Scale Version: input data: %s", byteInHex(VERpayload)));
                        resumeMachineState();
                        break;
                    case AH100_NOTIFICATION_MEASUREMENT:
                        if (data[0] == (byte) 0xBD) {
                            LogManager.d(TAG, "Scale plain response received");
                        }
                        if (data[0] == (byte) 0xBC) {
                            LogManager.d(TAG, "Scale encoded response received");
    ...
    private void AHcmdDate() {
        ...
        ConverterUtils.toInt16Le(date, 0, year);
        LogManager.d(TAG, String.format("AH100::AHcmdDate: data to send: %s", byteInHex(date)));
    ...
    private void AHcmdUserInfo(int weight) {
        ...
        ConverterUtils.toInt16Le(user, 3, weight);
...
//        AHsendCommand(AH100_CMD_SELECT_USER, pl);
    ...
    private void AHrcvEncodedMeasurement(byte[] encdata, byte[] encdata2, byte type) {
        ...
        try{
            ...
            LogManager.d(TAG, String.format("Decrypted measurement:  hex data: %s", byteInHex(data)));
            if (  (type == AH100_NOTIFICATION_MEASUREMENT) ||
                  (type == AH100_NOTIFICATION_HISTORY_RECORD) )   {
                AHaddFatMeasurement(data);
            }
        } catch (Exception e) {
            LogManager.d(TAG, "Decrypting FAIL!!!");
        }
    }

    private void AHaddFatMeasurement(byte[] data) {
        if (data.length < 14) {
            LogManager.d(TAG, ":: AHaddFatMeasurement : data is too short. Expected at least 14 bytes of data." );
            return ;
        }
        byte userid     = data[0]; ///// Arrays.copyOfRange(data, 0, 0 );
        lastMeasuredWeight = ConverterUtils.fromUnsignedInt16Le(data, 1);
        float weight     = lastMeasuredWeight / 10.0f;
        float fat        = ConverterUtils.fromUnsignedInt16Le(data, 3) / 10.0f;
        int year       = ConverterUtils.fromUnsignedInt16Le(data, 5) ;
        int resistance = ConverterUtils.fromUnsignedInt16Le(data, 13) ;
        ...
        LogManager.d(TAG, String.format("---- measured   userid     %d",userid     ));
        LogManager.d(TAG, String.format("---- measured  weight      %f",weight     ));
        LogManager.d(TAG, String.format("---- measured  fat         %f",fat        ));
        LogManager.d(TAG, String.format("---- measured  resistance  %d",resistance ));
        LogManager.d(TAG, String.format("---- measured  year        %d",year       ));
        LogManager.d(TAG, String.format("---- measured  month       %d",month      ));
        LogManager.d(TAG, String.format("---- measured  dayInMonth  %d",dayInMonth ));
        LogManager.d(TAG, String.format("---- measured  hour        %d",hour       ));
        LogManager.d(TAG, String.format("---- measured  minute      %d",minute     ));
        LogManager.d(TAG, String.format("---- measured  second      %d",second     ));
        LogManager.d(TAG, String.format("---- measured  week day    %d",weekNumber ));
        ///////////////////////////
...
// todo: calculate water, muscle, bones
    ...
    private void startHeartBeat() {
        LogManager.d(TAG, "*** Heart beat started");
        beatHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                LogManager.d(TAG, "*** heart beat.");
                AHcmdHeartBeat();
            }
        }, 2000); // 2 s
    }
    ...
    private void resetHeartBeat() {
        LogManager.d(TAG, "*** 0 heart beat reset");
        beatHandler.removeCallbacksAndMessages(null);
        startHeartBeat();
    }

    private void stopHeartBeat() {
        LogManager.d(TAG, "*** ! heart beat stopped");
        beatHandler.removeCallbacksAndMessages(null);
    }
    ...
    private void AHsendCommand(byte cmd, byte[] payload, int len ) {
        ...
        try {
            ...
            LogManager.d(TAG, "AHsendCommand: CANNOT WRITE COMMAND");
            stopHeartBeat();
        }
    }
    ...
    private void AHsendEncryptedCommand(byte cmd, byte[] payload , int len ) {
        ...
        byte[] header;
        byte[] encrypted;
        LogManager.d(TAG, String.format("AHsendEncryptedCommand: input data: %s", byteInHex(payload)));
        ...
        try {
            ...
            LogManager.d(TAG, "AHsendEncryptedCommand: CANNOT WRITE COMMAND");
            stopHeartBeat();
        }
    }
...
/////        return getfakeUserID();
    ...
    public byte[] encryptAES(byte[] data, byte[] key, byte[] ivs) {
        LogManager.d(TAG, String.format("Encoding   : input hex data: %s", byteInHex(data)));
        LogManager.d(TAG, String.format("Encoding   : encoding key  : %s", byteInHex(key)));
        LogManager.d(TAG, String.format("Encoding   : initial value : %s", byteInHex(ivs)));
        try {
    ...
    public byte[] decryptAES(byte[] data, byte[] key, byte[] ivs) {
        LogManager.d(TAG, String.format("Decoding   : input hex data: %s", byteInHex(data)));
        LogManager.d(TAG, String.format("Decoding   : encoding key  : %s", byteInHex(key)));
        LogManager.d(TAG, String.format("Decoding   : initial value : %s", byteInHex(ivs)));
        try {
            Cipher cipher = Cipher.getInstance("AES/CTR/NoPadding");
            SecretKeySpec secretKeySpec = new SecretKeySpec(key, "AES");
            byte[] finalIvs = new byte[16];
            ...
            LogManager.d(TAG, String.format("### decryptAES   :  hex data: %s", byteInHex(ret)));
            return ret;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public byte[] getPayload(byte[] data) {
        ...
        LogManager.d(TAG, String.format("Deobfuscated payload: %s", byteInHex(payload)));
        return payload;
    }
    ...
    private byte[] obfuscate(byte[] rawdata) {
        ...
        LogManager.d(TAG, String.format("Obfuscation: input hex data: %s", byteInHex(data)));
        //LogManager.d(TAG, "Obfuscation:  MAC  hex data: %s", byteInHex(MAC));
        ...
        //LogManager.d(TAG, "Obfuscation:  out  hex data: %s", byteInHex(data));
        return data;
    }
