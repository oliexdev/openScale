...
public class BluetoothIhealthHS3 extends BluetoothCommunication {
    private final String TAG = "BluetoothIhealthHS3";
    ...
    protected boolean onNextStep(int stepNr) {
        LogManager.d(TAG, "ihealthHS3 - onNextStep - returning false");
        return false;
    }
    ...
    public void disconnect() {
        ...
        LogManager.d(TAG, "HS3 - disconnect");
        if (btSocket != null) {
            if (btSocket.isConnected()) {
                try {
                    btSocket.close();
    ...
    private boolean sendBtData(String data) {
        LogManager.d(TAG, String.format("ihealthHS3 - sendBtData %s", data));
        if (btSocket.isConnected()) {
            btConnectThread = new BluetoothConnectedThread();
        ...
        LogManager.d(TAG, "ihealthHS3 - sendBtData - socket is not connected");
        return false;
    }
...
//            LogManager.d(TAG, "ihealthHS3 - BluetoothConnectedThread");
            isCancel = false;
...
//            LogManager.d(TAG, "ihealthHS3 - run");
...
//                    LogManager.d(TAG, "iheathHS3 - seen a byte "+String.format("%02X",btByte));
                   ...
                   if ( btByte == (byte) 0xA0 ) {
                     ...
                     if ( btByte == (byte) 0x09 ) {
                        ...
                        if ( btByte == (byte) 0xa6 ) {
                           btByte = (byte) btInStream.read();
                           if ( btByte == (byte) 0x28 ) {
//                              LogManager.d(TAG, "seen 0xa009a628 - Weight packet");
...
// and the weight - which should follow
                                 ...
                                 LogManager.d(TAG, "seen 0xa009a633 - time packet");
                                 // deal with a time packet, if needed
                                 } else {
                                 LogManager.d(TAG, String.format("iHealthHS3 - seen byte after control leader %02X", btByte));
                                 }
                                 }
                             }
                        }
...
//            LogManager.d(TAG, "iHealthHS3 - ScaleMeasurement "+String.format("%02X",weightBytes[0])+String.format("%02X",weightBytes[1]));
...
//            LogManager.d(TAG, "iHealthHS3 - ScaleMeasurement "+String.format("%f",weight));
...
//                LogManager.d(TAG, "iHealthHS3 - parseWeightArray returning null");
                return null;
                }     
