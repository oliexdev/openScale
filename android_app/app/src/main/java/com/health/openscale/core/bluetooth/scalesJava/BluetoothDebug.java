...
public class BluetoothDebug extends BluetoothCommunication {
    ...
    private void logService(BluetoothGattService service, boolean included) {
        LogManager.d(TAG, String.format("Service %s%s", BluetoothGattUuid.prettyPrint(service.getUuid()),
                included ? " (included)" : ""));

        for (BluetoothGattCharacteristic characteristic : service.getCharacteristics()) {
            LogManager.d(TAG, String.format("|- characteristic %s (#%d): %s%s",
            ...
            if (value != null && value.length > 0) {
                LogManager.d(TAG, String.format("|--> value: %s (%s)", byteInHex(value), byteToString(value)));
            }

            for (BluetoothGattDescriptor descriptor : characteristic.getDescriptors()) {
                LogManager.d(TAG, String.format("|--- descriptor %s%s",
