...
public class BluetoothOneByoneNew extends BluetoothCommunication{
    private final String TAG = "BluetoothOneByoneNew";
    ...
    public void onBluetoothNotify(UUID characteristic, byte[] data){
        if(data == null){
            LogManager.e(TAG, "Received an empty message", null);
            return;
        }

        LogManager.i(TAG, String.format("Received %s", new BigInteger(1, data).toString(16)));

        if(data.length < MSG_LENGTH){
            LogManager.e(TAG, "Received a message too short", null);
            return;
        }

        if(!(data[0] == HEADER_BYTES[0] && data[1] == HEADER_BYTES[1])){
            LogManager.e(TAG, "Unrecognized message received from scale.", null);
        }
        ...
        switch(data[2]){
            case (byte)0x00:
                ...
                if (data[7] != (byte)0x80){
                    LogManager.i(TAG, "Received real-time measurement. Skipping.");
                    break;
                }

                Date time = getTimestamp32(data, 3);
                weight = ConverterUtils.fromUnsignedInt24Be(data, 8) & 0x03ffff;
                weight /= 1000;
                ...
                LogManager.i(TAG, String.format("Added historic measurement. Weight: %s, impedance: %s, timestamp: %s", weight, impedance, time.toString()));
                break;

            case (byte)0x80:
                // final measurement
                currentMeasurement = new ScaleMeasurement();
                weight = ConverterUtils.fromUnsignedInt24Be(data, 3) & 0x03ffff;
                weight = weight / 1000;
                currentMeasurement.setWeight(weight);
                LogManager.d(TAG, String.format("Weight: %s", weight));
                break;
            case (byte)0x01:
                impedance = ConverterUtils.fromUnsignedInt16Be(data, 4);
                LogManager.d(TAG, String.format("impedance: %s", impedance));

                if(currentMeasurement == null){
                    LogManager.e(TAG, "Received impedance value without weight", null);
                    break;
                }
                ...
                break;
            default:
                LogManager.e(TAG, "Unrecognized message receveid", null);
        }
    }
    ...
    private void setMeasurementEntry(byte[] msg, int offset, int entryNum, int height, float weight, int sex, int age, int impedance, boolean impedanceLe){
        ...
        ConverterUtils.toInt16Be(msg, offset+2, roundedWeight);
    ...
    private void setTimestamp32(byte[] msg, int offset){
        ...
        ConverterUtils.toInt32Be(msg, offset, timestamp);
    }

    private Date getTimestamp32(byte[] msg, int offset){
        long timestamp = ConverterUtils.fromUnsignedInt32Be(msg, offset);
    ...
    private boolean setupMeasurementMessage(byte[] msg, int offset){
        ...
        ScaleMeasurement lastMeasure = getLastScaleMeasurement(userId);
        float weight = 0;
        int impedance = 0;
        if(lastMeasure != null){
            weight = lastMeasure.getWeight();
