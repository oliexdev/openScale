...
public class BluetoothMedisanaBS44x extends BluetoothCommunication {
    ...
    protected boolean onNextStep(int stepNr) {
        switch (stepNr) {
            ...
            case 3:
                ...
                byte[] date = ConverterUtils.toInt32Le(timestamp);
    ...
    private void parseWeightData(byte[] weightData) {
        float weight = ConverterUtils.fromUnsignedInt16Le(weightData, 1) / 100.0f;
        long timestamp = ConverterUtils.fromUnsignedInt32Le(weightData, 5);
        if (applyOffset) {
            timestamp += SCALE_UNIX_TIMESTAMP_OFFSET;
        }
    ...
    private float decodeFeature(byte[] featureData, int offset) {
        return (ConverterUtils.fromUnsignedInt16Le(featureData, offset) & 0x0FFF) / 10.0f;
    }
}
