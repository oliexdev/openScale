...
public class BluetoothActiveEraBF06 extends BluetoothCommunication {
    private static final String TAG = "BluetoothActiveEraBF06";
    ...
    private void sendConfigurationPacket() {
        ...
        LogManager.d(TAG, String.format("sending configuration packet: %s", byteInHex(packet)));
    ...
    protected boolean onNextStep(int stepNr) {
        switch (stepNr) {
            ...
            case 5: // HR measured! Maybe some historical will follow
                LogManager.i(TAG, "Measuring all done!");
    ...
    private void decodePacket(byte[] pkt) {
        if (pkt == null) {
            ...
            LogManager.d(TAG, "Wrong packet MAGIC");
            return;
        } else if (pkt.length != 20) {
            LogManager.d(TAG, String.format("Wrong packet length %s expected 20", pkt.length));
            return;
        }
        ...
        switch (packetType) {
            case 0xD5: // weight measurement
                ...
                float weightKg = (ConverterUtils.fromUnsignedInt24Be(pkt, 3) & 0x3FFFF) / 1000.0f;
                // TODO: test if it's always in grams ?
                if (stabilized && !weightStabilized) {
                    weightStabilized = true;
                    stableWeightKg = weightKg;
                    LogManager.i(TAG, String.format("Measured weight (stable): %.3f", stableWeightKg));
            ...
            case 0xD0: // balance measuring
                ...
                int weightLRaw = ConverterUtils.fromUnsignedInt16Be(pkt, 3);
                int percentLRaw = ConverterUtils.fromUnsignedInt16Be(pkt, 5);
                ...
                if (isFinal && !balanceStabilized) {
                    ...
                    LogManager.i(TAG, String.format("Measured balance (stable): L %.1f R: %.1f [%.2f]", percentL, 100.0f - percentL, weightL));
                    resumeMachineState();
                }
                break;
            ...
            case 0xD6: // reporting ADCs
                ...
                if (number == 1) {
                    double imp = ConverterUtils.fromUnsignedInt16Be(pkt, 4);
                    if (imp >= 1500.0d) {
                        imp = (((imp  - 1000.0d) + ((stableWeightKg * 10.0d) * (-0.4d))) / 0.6d) / 10.0d;
                    }
                    impedance = imp;
                    LogManager.i(TAG, String.format("Measured impedance: %.1f", impedance));
                    ...
                    LogManager.d(TAG, String.format("Unsupported number of ADCs: %s", number));
                }
            ...
            case 0xD7: // HR measured
                ...
                LogManager.i(TAG, String.format("Measured heart rate: %d", hr));
                resumeMachineState();
            ...
            default:
                LogManager.d(TAG, String.format("Unsupported packet [%d]: %s", packetType, byteInHex(pkt)));
        }
    ...
    private void calculateBIA(int heightCm, double impedanceOhm, float weightKg, int age, int gender) {
        ...
        LogManager.i(TAG, String.format("FFM: %.2f, FM: %.2f, BF: %.1f%%", fatFreeMass, fatMass, bodyFat));
    }

    private void parseHistoricalPacket(byte[] pkt) {
        Instant time = Instant.ofEpochSecond(ConverterUtils.fromUnsignedInt24Be(pkt, 3));
        float weight = (ConverterUtils.fromUnsignedInt24Be(pkt, 0x08) & 0x03FFFF) / 1000.0f;
        float weightLeft = ConverterUtils.fromUnsignedInt16Be(pkt, 0x0b) / 100.0f;
        int hr = pkt[0x0d] & 0xff;
        int adc = ConverterUtils.fromUnsignedInt16Be(pkt, 0x0f);
        LogManager.i(TAG, String.format("Historical measurement: %.3f kg, Weight Left: %.2f kg, HR: %d, ADC: %d", weight, weightLeft, hr, adc));
