...
public class BluetoothInlife extends BluetoothCommunication {
    private final String TAG = "BluetoothInlife";
    ...
    public void onBluetoothNotify(UUID characteristic, byte[] value) {
        ...
        if (data[0] != START_BYTE || data[data.length - 1] != END_BYTE) {
            LogManager.e(TAG, "Wrong start or end byte", null);
            return;
        }

        if (xorChecksum(data, 1, data.length - 2) != 0) {
            LogManager.e(TAG, "Invalid checksum", null);
            return;
        }

        if (Arrays.equals(data, lastData)) {
            LogManager.d(TAG, "Ignoring duplicate data");
            return;
        }
        lastData = data;
        ...
        switch (data[1]) {
            case (byte) 0x0f:
                LogManager.d(TAG, "Scale disconnecting");
                break;
            case (byte) 0xd8:
                float weight = ConverterUtils.fromUnsignedInt16Be(data, 2) / 10.0f;
            ...
            case (byte) 0xdd:
                ...
                break;
            case (byte) 0xdf:
                LogManager.d(TAG, String.format("User data acked by scale: %s", data[2] == 0 ? "OK" : "error"));
                break;
            default:
    ...
    void processMeasurementData(byte[] data) {
        float weight = ConverterUtils.fromUnsignedInt16Be(data, 2) / 10.0f;
        float lbm = ConverterUtils.fromUnsignedInt24Be(data, 4) / 1000.0f;
        float visceralFactor = ConverterUtils.fromUnsignedInt16Be(data, 7) / 10.0f;
        float bmr = ConverterUtils.fromUnsignedInt16Be(data, 9) / 10.0f;

        if (lbm == 0xffffff / 1000.0f) {
            LogManager.e(TAG, String.format("Measurement failed; feet not correctly placed on scale?"), null);
            return;
        }

        LogManager.d(TAG, String.format("weight=%.1f, LBM=%.3f, visceral factor=%.1f, BMR=%.1f",
    ...
    void processMeasurementDataNewVersion(byte[] data) {
        float weight = ConverterUtils.fromUnsignedInt16Be(data, 2) / 10.0f;
        long impedance = ConverterUtils.fromUnsignedInt32Be(data, 4);
        LogManager.d(TAG, String.format("weight=%.2f, impedance=%d", weight, impedance));
