...
public class BluetoothOKOK extends BluetoothCommunication {
    private static final String TAG = "BluetoothOKOK";
        ...
        public void onDiscoveredPeripheral(@NotNull BluetoothPeripheral peripheral, @NotNull ScanResult scanResult) {
            ...
            if (manufacturerSpecificData.indexOfKey(MANUFACTURER_DATA_ID_V20) > -1) {
                ...
                if (data[IDX_V20_CHECKSUM] != checksum) {
                    LogManager.d(TAG, String.format("Checksum error, got %x, expected %x", data[IDX_V20_CHECKSUM] & 0xff, checksum & 0xff));
                    return;
                }
                ...
                LogManager.d(TAG, String.format("Got weight: %f and impedance %f", weight / divider, impedance / 10f));
                ...
                if (data[IDX_V11_CHECKSUM] != checksum) {
                    LogManager.d(TAG, String.format("Checksum error, got %x, expected %x", data[IDX_V11_CHECKSUM] & 0xff, checksum & 0xff));
                    return;
                }
                ...
                default:
                    LogManager.w(TAG,"Invalid weight scale received, assuming 1 decimal", null);
                    /* fall-through */
                case 0:
                    divider = 10.0f;
                    break;
                ...
                LogManager.d(TAG, String.format("Got weight: %f", weight / divider));
                ScaleMeasurement entry = new ScaleMeasurement();
    ...
    public void connect(String macAddress) {
        LogManager.d(TAG, String.format("Mac address: %s", macAddress));
