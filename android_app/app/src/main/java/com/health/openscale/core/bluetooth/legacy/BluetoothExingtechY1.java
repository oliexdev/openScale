...
public class BluetoothExingtechY1 extends BluetoothCommunication {
    ...
    private void parseBytes(byte[] weightBytes) {
        ...
        float weight = ConverterUtils.fromUnsignedInt16Be(weightBytes, 4) / 10.0f; // kg
        float fat = ConverterUtils.fromUnsignedInt16Be(weightBytes, 6) / 10.0f; // %
        float water = ConverterUtils.fromUnsignedInt16Be(weightBytes, 8) / 10.0f; // %
        float bone = ConverterUtils.fromUnsignedInt16Be(weightBytes, 10) / 10.0f; // kg
        float muscle = ConverterUtils.fromUnsignedInt16Be(weightBytes, 12) / 10.0f; // %
        float visc_fat = weightBytes[14] & 0xFF; // index
        float calorie = ConverterUtils.fromUnsignedInt16Be(weightBytes, 15);
        float bmi = ConverterUtils.fromUnsignedInt16Be(weightBytes, 17) / 10.0f;
